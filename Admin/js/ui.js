export const labels={pending:'Chờ nhận',accepted:'Đã nhận',awaiting_pickup:'Chờ lấy hàng',picked_up:'Đã lấy hàng',delivering:'Đang giao',completed:'Hoàn thành',incomplete:'Chưa hoàn thành',cancelled:'Đã hủy'};
export const routes={same_province:'Nội tỉnh',same_region:'Nội miền',inter_region:'Liên miền'};
...
export function orderTable(items) {
  ...
  return `<div class="table-scroll"><table><thead><tr><th>Mã vận đơn</th><th>Người nhận</th><th>Dịch vụ</th><th>Trạng thái</th><th>Cước phí / COD</th><th>Nhân viên</th><th></th></tr></thead><tbody>${items.map(o => `<tr><td><button class="code-link" data-action="order-detail" data-id="${o.id}">${esc(o.tracking_code)}</button><small>${date(o.created_at)}</small></td><td><strong>${esc(o.receiver_name)}</strong><small>${esc(o.receiver_phone)}</small></td><td>${esc(o.service_name)}<small>${number(o.weight)} kg · ${routes[o.route_type] || 'Đơn cũ'}</small></td><td>${badge(o.status)}</td><td><strong>${money(o.total_amount ?? o.shipping_fee)}</strong><small>COD: ${money(o.cod_amount)}</small></td><td>${esc(o.employee_name || 'Chưa nhận')}</td><td><button class="icon-button" aria-label="Chi tiết ${esc(o.tracking_code)}" data-action="order-detail" data-id="${o.id}">${icon('arrow')}</button></td></tr>`).join('')}</tbody></table></div>`;
}
let toastTimer;
