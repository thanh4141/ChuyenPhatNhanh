...
dashboardRouter.get('/', async (req, res) => {
  const [[summary]] = await pool.query(`SELECT COUNT(*) AS total,
    ...
    COALESCE(SUM(status NOT IN ('completed','incomplete','cancelled','pending')),0) AS shipping,
    COALESCE(SUM(status='completed'),0) AS delivered,
    COALESCE(SUM(status='incomplete'),0) AS failed,
    COALESCE(SUM(CASE WHEN status='completed' THEN total_amount ELSE 0 END),0) AS revenue,
    COALESCE(SUM(CASE WHEN status='completed' THEN cod_amount ELSE 0 END),0) AS delivered_cod FROM orders`);
