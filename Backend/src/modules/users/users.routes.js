...
usersRouter.post('/', validate(z.object({ name: text(120), email, phone, password, role: z.enum(['employee','customer']), address: z.string().trim().max(500).default('') }).strict()), async (req, res) => {
...
usersRouter.patch('/:id', validate(z.object({ name: text(120).optional(), phone: phone.optional(), address: z.string().trim().max(500).optional(), active: z.boolean().optional() }).strict().refine(v => Object.keys(v).length > 0)), async (req, res) => {
  ...
  const user = await transaction(async connection => {
    ...
    if (target.role === 'employee' && req.input.active === false) {
      const [[{ count }]] = await connection.execute("SELECT COUNT(*) AS count FROM orders WHERE employee_id=? AND status NOT IN ('delivered','cancelled')", [userId]);
      if (count > 0) throw new AppError(409, 'Nhân viên còn đơn đang xử lý. Hãy phân công lại hoặc hoàn tất đơn trước.');
    }
    const fields = Object.keys(req.input);
    const revoke = req.input.active === false ? ', token_version=token_version+1' : '';
    await connection.execute(`UPDATE users SET ${fields.map(key => `${key}=?`).join(',')}${revoke} WHERE id=?`, [...Object.values(req.input), userId]);
    return { ...target, ...req.input };
  });
  res.json({ user });
});
