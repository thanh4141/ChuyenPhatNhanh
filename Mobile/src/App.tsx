...
type Tab='orders'|'waiting'|'create'|'notifications'|'profile';
function AuthenticatedApp(){
  const {user}=useAuth();const [tab,setTab]=useState<Tab>(user?.role==='employee'?'waiting':'orders');const [detail,setDetail]=useState<number|null>(null);
  const tabs:{id:Tab;label:string;symbol:string}[]=user?.role==='employee'?[{id:'waiting',label:'Đơn đang chờ',symbol:'◇'},{id:'orders',label:'Đơn đã nhận',symbol:'▤'},{id:'notifications',label:'Thông báo',symbol:'◌'},{id:'profile',label:'Cá nhân',symbol:'◉'}]:[{id:'create',label:'Tạo đơn',symbol:'＋'},{id:'orders',label:'Lịch sử',symbol:'▤'},{id:'notifications',label:'Thông báo',symbol:'◌'},{id:'profile',label:'Cá nhân',symbol:'◉'}];
  return <><View style={local.header}><View style={local.brand}><Text style={local.logo}>CPN</Text><Text style={local.brandText}>Chuyển Phát Nhanh</Text></View><View style={local.role}><Text style={local.roleText}>{user?.role==='employee'?'NHÂN VIÊN':'KHÁCH HÀNG'}</Text></View></View><View style={{flex:1}}>{detail!==null?<OrderDetailScreen id={detail} onBack={()=>setDetail(null)}/>:(tab==='orders'||tab==='waiting')?<OrdersScreen key={tab} scope={tab==='waiting'?'waiting':'mine'} onOpen={setDetail} onCreate={()=>setTab('create')}/>:tab==='create'&&user?.role==='customer'?<CreateOrderScreen onCreated={id=>{setTab('orders');setDetail(id);}}/>:tab==='notifications'?<NotificationsScreen onOpen={setDetail}/>:<ProfileScreen/>}</View><View style={local.tabs}>{tabs.map(t=><Pressable key={t.id} accessibilityRole="tab" accessibilityState={{selected:tab===t.id}} onPress={()=>{setDetail(null);setTab(t.id);}} style={local.tab}><Text style={[local.tabSymbol,tab===t.id&&{color:colors.green}]}>{t.symbol}</Text><Text style={[local.tabText,tab===t.id&&{color:colors.green,fontWeight:'700'}]}>{t.label}</Text>{tab===t.id&&<View style={local.tabDot}/>}</Pressable>)}</View></>;
}
